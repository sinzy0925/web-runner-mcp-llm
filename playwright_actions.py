# --- ファイル: playwright_actions.py (content取得結果格納 再確認・修正版) ---
import asyncio
import logging
import os
import time
...
async def _extract_emails_from_page_async(context: BrowserContext, url: str, timeout: int) -> List[str]:
    ...
    try:
        ...
        if emails_found:
            ...
            try:
                output_dir = Path("output") # config.py を使うのが望ましい
                output_dir.mkdir(exist_ok=True)
                mail_file_path = output_dir / "server_mails.txt" # config.py を使うのが望ましい
...
async def try_locators_sequentially(
    ...
    for i, hint in enumerate(hints):
        ...
        try:
            ...
            if hint_type == "role_and_text" and isinstance(hint_value, str):
                 role = hint_value # ★★★ 注意: LLM応答によっては role ではなく value にロール名が入る ★★★
                 # --- ▼▼▼ LLM応答に合わせて修正 ▼▼▼ ---
                 # LLMが {"type": "role_and_text", "name": "Google 検索", "confidence": "high", "value": "button"}
                 # のように返すケースに対応 (valueにロール名が入る)
                 actual_role = hint_value if isinstance(hint_value, str) else hint.get("role") # valueかroleキーを見る
                 if not isinstance(actual_role, str) or not actual_role:
                      logging.warning(f"    Invalid role value provided: {actual_role}")
                      attempt_log["reason"] = f"Invalid role value: {actual_role}"; raise ValueError("Role is missing or invalid")
                 ...
                 if isinstance(hint_name, str):
                     if hint_name.startswith('/') and hint_name.endswith('/') and len(hint_name) > 2:
                          try:
                              ...
                              logging.debug(f"    Using get_by_role('{actual_role}') with regex name: {options['name']}")
                          except re.error as re_err:
                              logging.warning(f"    Invalid regex pattern: {hint_name} - {re_err}. Treating as literal.")
                              options["name"] = hint_name # 通常文字列として扱う
                     else: options["name"] = hint_name; logging.debug(f"    Using get_by_role('{actual_role}') with exact name: {options['name']}")
            ...
            if locator_to_try:
                ...
                if count == 1:
                    ...
                    if is_visible:
                        ...
                        attempt_log["status"] = "Success"; attempt_log["reason"] = "Found unique and visible."; logging.info(f"    -> Success!")
                ...
                elif count > 1: attempt_log["status"] = "Fail"; attempt_log["reason"] = f"Multiple elements ({count})."; logging.info(f"    -> Fail: Multiple elements.")
                else: attempt_log["status"] = "Fail"; attempt_log["reason"] = "Not found."; logging.info(f"    -> Fail: Not found.")
            else: attempt_log["status"] = "Fail" # Locator生成失敗
        except ValueError as ve: # role_and_text の role 不正など
            attempt_log["status"] = "Error"; attempt_log["reason"] = f"ValueError: {ve}"; logging.warning(f"    -> ValueError: {ve}")
    ...
    else: logger.warning(f"[{action_name}] Failed to locate element using hints.")
    return successful_locator, successful_hint, attempt_logs


# --- execute_actions_async 本体 (content取得結果格納 再確認・修正版) ---
    ...
    for i, step_data in enumerate(actions):
        ...
        try:
            ...
            if action == "click":
                ...
                pdf_text_content = None; processed_value = attr_value; scraped_content = None # ★ scraped_content 追加
                ...
                # --- ▼▼▼ content 取得処理を追加 ▼▼▼ ---
                     ...
                     try:
                         ...
                         if parsed_url.scheme in ['http', 'https'] and not absolute_url.lower().endswith('.pdf'):
                              ...
                              scraped_content = content_or_error # 成功しても失敗しても内容を記録
                         elif absolute_url.lower().endswith('.pdf'):
                              scraped_content = "Warning: Link is a PDF, use attribute_name='pdf' instead."
                         else:
                              scraped_content = f"Warning: Invalid URL scheme or not HTTP/S: {absolute_url}"
                ...
                if scraped_content is not None: action_result_details["scraped_text"] = scraped_content # ★ scraped_text を追加
                results.append({**step_result_base, "status": "success", **action_result_details})
                ...
                if not found_elements_list: logger.warning(f"No elements for '{selector}'."); action_result_details["results_count"] = 0
                    ...
                    if attribute_name.lower() in ['href', 'pdf', 'content', 'mail']:
                        ...
                        async def process_single_element_for_href_related(locator: Locator, index: int, base_url: str, attr_mode: str, sem: asyncio.Semaphore) -> Tuple[Optional[str], Optional[str], Optional[str], Optional[List[str]]]:
                            ...
                            async with sem:
                                try:
                                    ...
                                    if original_href is None: return None, None, None, None # hrefなければスキップ
                                    ...
                                    # 有効なURLスキームかチェック
                                    if parsed_url.scheme not in ['http', 'https']: return absolute_url, None, None, None # URLは記録するが以降の処理はスキップ
                                    ...
                                    # --- ▼▼▼ content取得ロジック修正 ▼▼▼ ---
                                         ...
                                         success_content, content_or_error = await get_page_inner_text(current_context, absolute_url, action_wait_time);
                                         scraped_text = content_or_error # 成功/失敗に関わらず結果を保持
                                    # --- ▲▲▲ ---
                                ...
                                # locator.get_attribute や asyncio.to_thread などで発生しうるエラーをキャッチ
                                except (PlaywrightError, Exception) as e:
                                    logger.warning(f"Error processing element {index} for {attr_mode} ({absolute_url}): {type(e).__name__}")
                                    # エラーの場合でも、URLがあればそれを返し、他はNoneやエラーメッセージにする
                                    error_msg = f"Error: {type(e).__name__}"
                                    return absolute_url or f"Error processing: {original_href}", \
                                           pdf_text if pdf_text else error_msg if attr_mode=='pdf' else None, \
                                           scraped_text if scraped_text else error_msg if attr_mode=='content' else None, \
                                           emails_from_page if emails_from_page else [error_msg] if attr_mode=='mail' else None
                        ...
                        for url, pdf, content, mails in results_tuples:
                            ...
                            if attribute_name.lower() == 'content': content_list.append(content) # ★ここが重要★
                        ...
                        action_result_details["url_list"] = url_list # URLリストは常に含める

                        if attribute_name.lower() == 'pdf': action_result_details["pdf_texts"] = pdf_list
                        # ★★★ content_list を scraped_texts として格納 ★★★
                ...
                logger.info("Element visibility confirmed."); results.append({**step_result_base, "status": "success", **action_result_details})
                  ...
                  elif option_type == 'label': value_to_select = {'label': str(option_value)}
                  selected_values = await target_element.select_option(value_to_select, timeout=action_wait_time) # 戻り値も取得
                  ...
                  ss_timeout = max(10000, action_wait_time) # スクショは最低10秒
                  ...
                  else: await root_page.screenshot(path=screenshot_path, full_page=True, timeout=ss_timeout*2); logger.info("Page screenshot saved.") # ページ全体は長めに
        ...
        # --- ステップごとのエラーハンドリング ---
            ...
            if root_page and not root_page.is_closed():
                 ...
                 try:
                     ...
                     await root_page.screenshot(path=error_ss_path, full_page=True, timeout=15000) # エラー時SSタイムアウト
            ...
            error_details = { **step_result_base, "status": "error", "selector": selector, "target_hints": target_hints, "locator_attempts": locator_attempt_logs, "message": str(e), "full_error": error_message, "traceback": traceback.format_exc(),}
